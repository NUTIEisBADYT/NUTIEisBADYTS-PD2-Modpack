local function _get_skills_perk_text(peer)
	...
	if #s == 15 then
		...
		abb_len = 5
		local perk_name = man_loc:text('menu_st_spec_' .. p[1])
...
function TeamLoadoutItem:set_slot_outfit(slot, criminal_name, outfit)
	...
	local txt = outfit and LobbyPlayerInfo.settings.show_perkdeck_in_loadout and 'st_spec_' .. tostring(outfit.skills.specializations[1]) or criminal_name
