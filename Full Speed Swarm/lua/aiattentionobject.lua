...
function AIAttentionObject:get_attention(filter, min, max, team)
	if self._attention_data then
		local rel_cache_key = (min or REACT_MIN) .. filter .. (max or REACT_MAX) .. (team and team.id or "")
		local rel_cache = self.rel_cache
		local result = rel_cache[rel_cache_key]
		if result == nil then
			result = fs_original_aiattentionobject_getattention(self, filter, min, max, team)
			rel_cache[rel_cache_key] = result or false
		end
		return result
	end
end
