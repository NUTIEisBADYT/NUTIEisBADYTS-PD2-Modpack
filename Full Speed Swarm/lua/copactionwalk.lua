...
CopActionWalk._move_speeds = {}
local function build_move_speeds()
	for character, tweaks in pairs(tweak_data.character) do
		if type(tweaks) == 'table' and tweaks.move_speed and character ~= 'presets' then
			CopActionWalk._move_speeds[character] = CopActionWalk._move_speeds[character] or {}
			for pose, stances in pairs(tweaks.move_speed) do
				for variant, walk_dirs in pairs(stances) do
					for stance, variants in pairs(walk_dirs) do
						CopActionWalk._move_speeds[character][stance] = CopActionWalk._move_speeds[character][stance] or {}
						CopActionWalk._move_speeds[character][stance][variant] = CopActionWalk._move_speeds[character][stance][variant] or {}
						for walk_dir, value in pairs(variants) do
							CopActionWalk._move_speeds[character][stance][variant][walk_dir] = CopActionWalk._move_speeds[character][stance][variant][walk_dir] or {}
							CopActionWalk._move_speeds[character][stance][variant][walk_dir][pose] = value
						end
						CopActionWalk._move_speeds[character][stance][variant].l = CopActionWalk._move_speeds[character][stance][variant].strafe
						CopActionWalk._move_speeds[character][stance][variant].r = CopActionWalk._move_speeds[character][stance][variant].strafe
					end
				end
			end
		end
...
function CopActionWalk:init(action_desc, common_data)
	...
	self._move_speed = self._move_speeds[tt][common_data.stance.name][action_desc.variant]
...
function CopActionWalk:_get_current_max_walk_speed(move_dir)
	return self._move_speed[move_dir][self._ext_anim.pose] * self._ext_movement.move_speed_multiplier
end
...
function CopActionWalk:update(t)
	...
	elseif not self._expired then
		...
		local need_update = self.fs_changing > 0
		if need_update then
		...
		if not move_dir_norm then
			...
			move_dir_norm = move_dir:normalized()
		...
		if variant == 'run' and not self._no_walk then
			...
			elseif self._ext_anim.run then
				if real_velocity > 530 and self._walk_anim_velocities[pose] and self._walk_anim_velocities[pose][self._stance.name] and self._walk_anim_velocities[pose][self._stance.name].sprint and self._ext_anim.pose == 'stand' then
					variant = 'sprint'
				elseif real_velocity > 250 then
					variant = 'run'
				else
		...
		local anim_walk_speed = self._move_speed[wanted_walk_dir][pose]
