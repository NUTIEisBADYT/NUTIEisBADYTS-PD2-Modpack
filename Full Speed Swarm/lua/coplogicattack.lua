...
function CopLogicAttack._upd_combat_movement(data)
	...
	elseif not enemy_visible_soft then
		...
		elseif my_data.at_cover_shoot_pos then
			if not my_data.stay_out_time then
				my_data.stay_out_time = my_data.attitude == "engage" and (t + 7) or t
			end
	...
	if data.important and not my_data.turning and not data.unit:movement():chk_action_forbidden("walk") and CopLogicAttack._can_move(data) and focus_enemy.verified and (not in_cover or not in_cover[4]) then
