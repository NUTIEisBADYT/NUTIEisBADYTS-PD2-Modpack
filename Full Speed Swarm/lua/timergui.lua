...
function TimerGui:init(...)
	...
	self.localized_prop_timer_gui_seconds = managers.localization:text("prop_timer_gui_seconds")
end
...
function TimerGui:update(unit, t, dt)
	...
	local dt_mod = math_max(self._timer_multiplier or 1, 0.01)
	...
	gui_script.time_text:set_text(math_floor(self._time_left or self._current_timer) .. ' ' .. self.localized_prop_timer_gui_seconds)
