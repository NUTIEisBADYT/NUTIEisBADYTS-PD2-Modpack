...
local fastpaced = FullSpeedSwarm.settings.fastpaced
local function SetImportantCops()
	fastpaced = FullSpeedSwarm.settings.fastpaced
end
table.insert(FullSpeedSwarm.call_on_fastpaced, SetImportantCops)

function CopLogicTravel.queue_update(data, my_data, delay)
	if fastpaced or data.important then
		delay = 0.1
	else
		delay = delay or 0.3
	end
