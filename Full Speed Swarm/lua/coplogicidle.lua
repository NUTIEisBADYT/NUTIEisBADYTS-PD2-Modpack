...
local function SetLoud()
	...
	if FullSpeedSwarm.settings.instant_identify then
		...
		elseif attention_data.identified then
			...
			if not reaction_too_mild then
				...
				if attention_data.is_local_player then
					...
					if aum and not aum._move_data and aum._pose_code == 2 then
						local aub = att_unit:base()
						if aub and aub.upgrade_value then
							weight_mul = weight_mul * (aub:upgrade_value("player", "stand_still_crouch_camouflage_bonus") or 1)
							if aub.has_activate_temporary_upgrade and aub:has_activate_temporary_upgrade("temporary", "chico_injector") and aub:upgrade_value("player", "chico_preferred_target") then
								weight_mul = weight_mul * 1000
							end
