...
function ObjectInteractionManager:update(t, dt)
	...
	if self._interactive_count > 0 and alive(player_unit) then
		...
		self:_update_targeted(player_pos, player_unit, t)
	end
end
...
function ObjectInteractionManager:_update_targeted(player_pos, player_unit, t)
	if self._active_object_locked_data then
		if not alive(self._active_unit) or not self._active_unit:interaction():active() then
			...
			if self._active_unit:interaction():interact_dont_interupt_on_distance() or distance <= self._active_unit:interaction():interact_distance() then
				return
			end
		end
	end
	...
	local last_active, active_unit, closest_locator
	if alive(self._active_unit) then
		last_active = self._active_unit
	end
	...
	elseif self._active_locator and self._active_locator ~= last_active_locator then
		last_active:interaction():unselect()
		if not last_active:interaction():selected(player_unit, self._active_locator) then
			active_unit = nil
			self._active_locator = nil
		end
	elseif last_active and last_active:interaction():dirty() then
		last_active:interaction():set_dirty(false)
		last_active:interaction():unselect()
		if not last_active:interaction():selected(player_unit, self._active_locator) then
			active_unit = nil
		end
