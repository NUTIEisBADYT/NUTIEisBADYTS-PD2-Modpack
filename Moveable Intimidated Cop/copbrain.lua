...
function CopBrain:on_intimidated_follow_objective_failed(unit)
	if not unit:character_damage():dead() then
		if not self._logic_data.objective or self._logic_data.objective.is_default or self._current_logic_name == "intimidated" then
...
function CopBrain:on_intimidated(amount, aggressor_unit)
	if aggressor_unit and self._unit:base().mic_is_being_moved == aggressor_unit then
		...
		local wp = managers.hud and managers.hud._hud and managers.hud._hud.waypoints["CustomWaypoint_" .. (peer_id == managers.network:session():local_peer():id() and "localplayer" or peer_id)]
		local wp_position = wp and mvector3.copy(wp.position) or nil
		local old_dst = self._unit:base().mic_destination
		...
		elseif old_dst or not self._unit:anim_data().move then
		...
		CopLogicIdle.mic_reset_rebellion(self._logic_data)
