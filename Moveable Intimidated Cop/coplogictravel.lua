...
function CopLogicTravel.enter(data, new_logic_name, enter_params)
	local mic_is_being_moved = not not data.unit:base().mic_is_being_moved
	if not mic_is_being_moved then
	...
	my_data.is_hostage = mic_is_being_moved
...
function CopLogicTravel._upd_enemy_detection(data)
	return data.unit:base().mic_is_being_moved and CopLogicIdle.mic_check_surroundings(data) or mic_original_coplogictravel_updenemydetection(data)
end
...
function CopLogicTravel._on_destination_reached(data)
	if data.unit:base().mic_destination then
		...
		data.brain:on_hostage_move_interaction(data.unit:base().mic_is_being_moved, "fail")
		return
	end
